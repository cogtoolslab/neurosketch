...
if __name__ == "__main__":
    ...
    for i, path_to_file in enumerate(data_paths):
        ...
        if ((check_exists(s3, args.bucket_name, filename)==False) | (overwrite==True)):
            print('Now uploading {} | {} of {}'.format(path_to_file.split('/')[-1],(i+1),len(data_paths)))
            s3.Object(args.bucket_name,filename).put(Body=open(path_to_file,'rb')) ## upload stimuli
            s3.Object(args.bucket_name,filename).Acl().put(ACL='public-read') ## set access controls
        else: 
