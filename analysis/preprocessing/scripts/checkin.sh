...
do
  ...
  then
    if [ -f "subjects/${subject}/status" ]
    then
      line=$(head -n 1 subjects/${subject}/status)
      echo "${subject} - ${line}"
    else
      echo "${subject} information unavailable - check if subject analysis is running"
    fi
  else
