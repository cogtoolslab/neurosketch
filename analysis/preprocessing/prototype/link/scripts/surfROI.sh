...
FILLTHRESH=0 # threshold for incorporating a voxel in the mask, default = 0
REFERENCEFUNC=${NOW}/analysis/firstlevel/parameter/2mmExample.nii.gz
REFERENCEVOLUME=${NOW}/data/nifti/${SUBJ}_anat_mprage_brain.nii.gz
REGISTERDAT=${SUBJECTS_DIR}/freesurfer/register.dat
...
do
  ...
  for HEMI in lh rh
  do
    LABEL=${SUBJECTS_DIR}/freesurfer/label/${HEMI}.${ROI}_exvivo.label
...
ROIS="V1 V2 hipp_FS ento_FS parahippo_FS LOC_FS fusiform_FS IT_FS perirhinal"
